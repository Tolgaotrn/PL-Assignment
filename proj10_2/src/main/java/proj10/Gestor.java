		...
		for (Recursos recurso : recursos) {
			...
			switch (recurso.getTipo().getTipo()) {
				...
				default:
					...
					Query<DefaultType> queryDefault = session.createQuery("FROM DefaultType WHERE id = :id", DefaultType.class);
					queryDefault.setParameter("id", recurso.getIdRecurso());
					List<DefaultType> defaultTypes = queryDefault.getResultList();
					for (DefaultType defaultType : defaultTypes) {
						System.out.println("Tipo: " + recurso.getTipo().getTipo());
						System.out.println("Tipo de Comida: " + defaultType.getName());
						System.out.println("-------------------------------");
					}
					break;
			}
